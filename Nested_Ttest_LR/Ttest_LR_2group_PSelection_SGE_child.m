function Ttest_LR_2group_PSelection_SGE_child(SubjectsData_Path, Subjects_Label, i, P_Value_Range, Pre_Method, ResultantFolder)
%
% Copyright(c) 2016
...
for j = 1:length(P_Value_Range)
    Accuracy(j) = Ttest_LR_2group_ACC(All_Training, Label, P_Value_Range(j), Pre_Method);
end
...
predicted_label = t.classifyInstance(dw.instance(0));
if ~predicted_label
    predicted_label = -1;
end
decision_value = [1 test_data_New] * t.coefficients;
